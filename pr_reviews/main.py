...
def main():
    ...
    for repo in repo_names:
        ...
        timestamped_print(
            f"Finished get_pull_requests_between_dates {time.time() - start_time:.2f} seconds for {len(pull_requests)} pull requests"
        )
        ...
        for pr_numbers in pr_numbers_batched:
            reviewers = get_reviewers_for_pull_requests(org_name, repo, pr_numbers)
            for reviewer in reviewers:
                ...
                if reviewer["login"] in reviewer_prs:
                    ...
                    reviewer_prs[reviewer["login"]] = 1
        timestamped_print(
            f"Finished processing {repo} {time.time() - start_time:.2f} seconds"
        )
